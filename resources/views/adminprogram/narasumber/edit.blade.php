...
<div class="container mx-auto p-6 max-w-4xl">
    <!-- Header -->
    <div class="mb-8">
        <a href="{{ route('adminprogram.narasumber.index', $program->id) }}" class="inline-flex items-center text-sm text-gray-500 hover:text-indigo-600 transition-colors mb-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
        <form action="{{ route('adminprogram.narasumber.update', [$program->id, $narasumber->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
        ...
        <!-- Delete Form -->
        <form id="delete-form" action="{{ route('adminprogram.narasumber.destroy', [$program->id, $narasumber->id]) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
