...
<div class="container mx-auto p-6 max-w-4xl">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('adminprogram.kelas.edit', $module->kelas_id) }}" class="text-gray-500 hover:text-indigo-600 flex items-center gap-2">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 border border-gray-100 dark:border-gray-700">
        <form action="{{ route('adminprogram.modules.update', $module->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <div class="flex items-center pt-6">
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_mandatory" class="w-5 h-5 text-green-600 rounded focus:ring-green-500" @checked($module->is_mandatory)>
                        <div class="ml-3">
                            <span class="block text-sm font-bold text-gray-700 dark:text-gray-300">Wajib Dibaca?</span>
                            <span class="block text-xs text-gray-500">Jika dicentang, peserta harus membuka modul ini.</span>
                        </div>
                    </label>
                </div>
            </div>
        ...
        <form id="delete-form" action="{{ route('adminprogram.modules.destroy', $module->id) }}" method="POST" style="display: none;" onsubmit="return confirm('Yakin ingin menghapus modul ini?');">
            @csrf
            @method('DELETE')
        </form>
    </div>
