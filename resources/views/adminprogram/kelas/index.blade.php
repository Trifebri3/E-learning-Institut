...
<div class="container mx-auto p-6 max-w-6xl">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('adminprogram.kelas.create') }}"
           class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition font-medium flex items-center">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @foreach($kelas as $k)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                        ...
                        <td class="px-6 py-4">
                            <form action="{{ route('adminprogram.kelas.toggle-publish', $k->id) }}" method="POST">
                                @csrf
                                @method('PATCH')
                                @if($k->is_published)
                                    <button type="submit"
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-center space-x-2">
                                <!-- Edit Button -->
                                <a href="{{ route('adminprogram.kelas.edit', $k->id) }}"
                                ...
                                <!-- Print Button -->
                                <a href="{{ route('adminprogram.presensi.exportKelas', $k->id) }}"
                                   target="_blank"
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('adminprogram.kelas.destroy', $k->id) }}" method="POST"
                                      onsubmit="return confirm('Hapus kelas ini?')" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
