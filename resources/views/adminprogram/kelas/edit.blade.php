...
<div class="container mx-auto p-6 max-w-6xl" x-data="{ activeTab: 'info' }">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('adminprogram.kelas.index') }}" class="text-gray-500 hover:text-indigo-600 flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6">
        ...
        <div x-show="activeTab === 'info'" x-transition>
            <form action="{{ route('adminprogram.kelas.update', $kelas->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
        ...
        <div x-show="activeTab === 'presensi'" x-transition style="display: none;">
            <form action="{{ route('adminprogram.kelas.update', $kelas->id) }}" method="POST">
                @csrf @method('PUT')
                {{-- Bypass fields lain --}}
        ...
        <div x-show="activeTab === 'materi'" x-transition style="display: none;" class="space-y-6">
            ...
            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-800">
                <div class="flex justify-between items-center mb-3">
                    ...
                    @if(!$kelas->learningPath)
                        ...
                        <form action="{{ route('adminprogram.learningpath.store', $kelas->id) }}" method="POST">
                            @csrf
                    ...
                    @else
                        <!-- Tombol Kelola -->
                        <a href="{{ route('adminprogram.learningpath.manage', $kelas->learningPath->id) }}" class="text-sm bg-yellow-500 text-white px-3 py-1.5 rounded hover:bg-yellow-600 font-bold">
            ...
            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-800">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <a href="{{ route('adminprogram.modules.create', $kelas->id) }}" class="text-sm bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 transition">
                    ...
                    @forelse($kelas->modules as $mod)
                        <li class="flex justify-between items-center p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded border border-transparent hover:border-gray-200 dark:hover:border-gray-600 transition">
                            <div>
                                ...
                                <span class="ml-2 text-xs px-2 py-0.5 rounded-full {{ $mod->is_mandatory ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                    {{ $mod->is_mandatory ? 'Wajib' : 'Opsional' }}
                                </span>
                            </div>
                            <a href="{{ route('adminprogram.modules.edit', $mod->id) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
            ...
            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-800">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <a href="{{ route('adminprogram.videos.create', $kelas->id) }}" class="text-sm bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700 transition">
                ...
                <ul class="space-y-2">
                    @forelse($kelas->videoEmbeds as $vid)
                        <li class="flex justify-between items-center p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded border border-transparent hover:border-gray-200 dark:hover:border-gray-600 transition">
                            ...
                            <a href="{{ route('adminprogram.videos.edit', $vid->id) }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium whitespace-nowrap ml-2">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                        </li>
                    @empty
        ...
        <div x-show="activeTab === 'tugas'" x-transition style="display: none;" class="space-y-6">
            ...
            <!-- Tugas -->
            @include('adminprogram.assignments.components.list', ['assignments' => $assignments, 'kelas' => $kelas])
            ...
            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-800">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="{{ route('adminprogram.quizzes.create', $kelas->id) }}" class="text-sm bg-purple-600 text-white px-3 py-1.5 rounded hover:bg-purple-700 font-bold">
                        <i class="fas fa-plus mr-1"></i> Tambah Kuis
                    </a>
                </div>

                @include('adminprogram.quiz.components.list', ['quizzes' => $kelas->quizzes, 'kelas' => $kelas])
            </div>

            <!-- Ujian Essay -->
            @if(isset($kelas->essayExam) && $kelas->essayExam)
                @include('adminprogram.essay.components.card-summary', ['exam' => $kelas->essayExam])
            @else
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div class="text-center py-6">
                        ...
                        <a href="{{ route('adminprogram.essay.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow">
