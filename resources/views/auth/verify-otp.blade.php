...
<div class="mb-6 text-center">
    ...
    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
        Kami telah mengirimkan kode OTP 6 digit ke email Anda. Silakan masukkan kode tersebut untuk memverifikasi akun Anda.
    </p>
</div>
...
<div class="flex items-center justify-between">
    <form method="POST" action="{{ route('verification.send') }}">
        @csrf
        <button type="submit" class="text-sm text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300 transition-colors duration-200">
    ...
    <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" class="text-sm text-gray-600 hover:text-gray-500 dark:text-gray-400 dark:hover:text-gray-300 transition-colors duration-200">
