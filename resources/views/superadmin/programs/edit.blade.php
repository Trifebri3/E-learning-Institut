...
<div class="container mx-auto p-6 max-w-4xl">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Program: {{ $program->title }}</h1>
        <a href="{{ route('superadmin.programs.index') }}" class="text-gray-500 hover:text-blue-600">Kembali</a>
    </div>

    <form action="{{ route('superadmin.programs.update', $program->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="md:col-span-2 space-y-6">
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Kode Redeem</label>
                            <input type="text" name="redeem_code" value="{{ old('redeem_code', $program->redeem_code) }}" class="w-full rounded border-gray-300 dark:bg-gray-700 dark:text-white" required>
                        </div>
                        <div>
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi_lengkap" rows="5" class="w-full rounded border-gray-300 dark:bg-gray-700 dark:text-white">{{ old('deskripsi_lengkap', $program->deskripsi_lengkap) }}</textarea>
                    </div>
                </div>
            </div>
                            ...
                            @foreach($admins as $admin)
                                <label class="flex items-center space-x-2 p-1 hover:bg-gray-50 dark:hover:bg-gray-700 rounded cursor-pointer">
                                    <input type="checkbox" name="admin_ids[]" value="{{ $admin->id }}"
                                        @checked(in_array($admin->id, (array) $selectedAdmins))>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        @endforeach
                ...
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
                    ...
                    <div class="mb-4">
                        ...
                        @if($program->logo_path)
                            <img src="{{ Storage::url($program->logo_path) }}" class="h-16 w-16 object-cover rounded mb-2">
                        @endif
                    ...
                    <div class="mb-4">
                        ...
                        @if($program->banner_path)
                            <img src="{{ Storage::url($program->banner_path) }}" class="h-20 w-full object-cover rounded mb-2">
                        @endif
                ...
                <button type="submit" class="w-full px-6 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-lg">
