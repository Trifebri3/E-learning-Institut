...
<nav class="sidebar-content flex-1 px-3 py-4 overflow-y-auto scrollbar-thin">
    ...
    <ul class="space-y-1">
        ...
        <li class="pt-2">
            ...
            @php
    $programs = \App\Models\Program::whereIn('id', function($query) {
...
<li>
    <a href="{{ route('instructor.kelas.index') }}"
       class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300
              hover:bg-primary-50 dark:hover:bg-gray-700"
       :class="{
           'bg-primary-50 dark:bg-gray-700 border-l-4 border-primary-500 text-primary-600
            dark:text-primary-400 font-semibold':
            $page.url.startsWith('/instructor/kelas')
       }">
        <i class="fa-solid fa-users w-5 text-center"></i>
        <span class="ml-3 sidebar-text">Manajemen kelas</span>
    </a>
</li>

<li>
    <a href="{{ route('instructor.resources.index') }}"
       class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700
              {{ request()->routeIs('instructor.resources.*') ? 'bg-primary-50 dark:bg-gray-700 border-l-4 border-primary-500 text-primary-600 dark:text-primary-400 font-semibold' : '' }}">
        <i class="fa-solid fa-chalkboard-teacher w-5 text-center"></i>
        <span class="ml-3 sidebar-text">Kelola Materi</span>
    </a>
</li>
<!-- Kelola Ujian & Tugas -->
<li x-data="{ open: false }">
    <a href="javascript:void(0);"
       ...
       :class="{
           'bg-primary-50 dark:bg-gray-700 border-l-4 border-primary-500 text-primary-600
            dark:text-primary-400 font-semibold': open
       }">
        <i class="fa-solid fa-file-alt w-5 text-center"></i>
        <span class="ml-3 sidebar-text">Kelola Ujian & Tugas</span>
        <i class="fa-solid fa-chevron-down ml-auto"></i>
    </a>
    <ul x-show="open" class="pl-8 mt-1 space-y-1" style="display: none;">
        <li>
            <a href="{{ route('instructor.quiz.index') }}"
               class="block p-2 rounded-lg text-gray-600 hover:bg-primary-50 dark:text-gray-400 dark:hover:bg-gray-700">
               Kelola Ujian Pilihan Ganda
            </a>
        </li>
        <li>
        ...
        <li>
            <a href="{{ route('instructor.essay.index') }}"
               ...
               Kelola Ujian Tulis
                ...
                <li>
                    <a href="{{ route('instructor.announcements.index') }}" class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700"
...
@php
    ...
    // Cek administeredPrograms (adminprogram/instructor)
    if ($user && method_exists($user, 'administeredPrograms')) {
        $programs = $user->administeredPrograms()->get();
    }

    // Jika relasi instructedPrograms ada, gabungkan
    if ($user && method_exists($user, 'instructedPrograms')) {
        $programs = $programs->merge($user->instructedPrograms()->get())->unique('id');
    }
@endphp

{{-- Jika tidak punya program sama sekali --}}
@if($programs->isEmpty())
    <div class="flex items-center p-3 rounded-lg text-gray-400 dark:text-gray-600 cursor-not-allowed"
         title="Anda belum mengelola program apapun">
       <i class="fas fa-users w-5 text-center"></i>
       <span class="ml-3">Narasumber</span>
       <span class="ml-2 text-xs bg-gray-200 dark:bg-gray-700 px-1 rounded">Tidak tersedia</span>
    </div>

{{-- Jika hanya 1 program → langsung auto-link --}}
@elseif($programs->count() === 1)
    <a href="{{ route('instructor.narasumber.index', $programs->first()->id) }}"
       class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700">
       <i class="fas fa-users w-5 text-center"></i>
       <span class="ml-3">Narasumber</span>
    </a>

{{-- Jika punya banyak program → tampilkan dropdown --}}
@else
    <div x-data="{ open: false }" class="w-full">
        <button @click="open = !open"
                class="flex items-center w-full p-3 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700">
            <i class="fas fa-users w-5 text-center"></i>
            <span class="ml-3">Narasumber</span>
        ...
        <div x-show="open" x-transition class="ml-8 mt-2 space-y-2">
            @foreach($programs as $p)
                <a href="{{ route('instructor.narasumber.index', $p->id) }}"
                   class="block px-3 py-2 text-sm rounded hover:bg-primary-100 dark:hover:bg-gray-700">
                    {{ $p->title }}
                </a>
            @endforeach
        </div>
...
<a href="{{ route('instructor.progress.index') }}"
   class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700"
   :class="{ 'bg-primary-50 dark:bg-gray-700 border-l-4 border-primary-500 text-primary-600 dark:text-primary-400 font-semibold': $page.url.startsWith('/eraport') }">
    <i class="fa-solid fa-life-alt w-5 text-center"></i>
    <span class="ml-3 sidebar-text">E-Raport</span>
</a>
        ...
        <!-- Diskusi -->
        <li>
<a href="{{ route('instructor.discussion.index') }}"
   class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700"
   :class="{ 'bg-primary-50 dark:bg-gray-700 border-l-4 border-primary-500 text-primary-600 dark:text-primary-400 font-semibold': $page.url.startsWith('/discussion') }">
    ...
    <span class="ml-3 sidebar-text">Diskusi</span>
        ...
        <!-- Support -->
        <li>
            <a href="{{ route('instructor.support.index') }}"
               class="flex items-center p-3 rounded-lg menu-item text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-gray-700"
