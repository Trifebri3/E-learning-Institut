...
<div class="container mx-auto p-6 max-w-4xl">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('instructor.essay.submissions', $submission->essay_exam_id) }}" class="text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-100 dark:border-gray-700 p-6">
            ...
            <form action="{{ route('instructor.essay.answers.grade', $ans->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT') {{-- Karena route menggunakan PUT --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <textarea name="feedback" rows="3" class="w-full rounded border-gray-300 dark:bg-gray-700 dark:text-white"
                                  placeholder="Berikan feedback...">{{ $ans->notes }}</textarea>
                    </div>
                </div>
    ...
    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
        {{-- PERBAIKAN: Gunakan route name yang benar --}}
        <form action="{{ route('instructor.essay.submissions.finish-grading', $submission->id) }}" method="POST">
            @csrf
            @method('PUT') {{-- Karena route menggunakan PUT --}}
