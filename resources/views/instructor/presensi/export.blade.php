    ...
    <div class="meta">
        <table>
            <tr>
                ...
                <td style="border:none;">: {{ $kelas->jam_mulai }} - {{ $kelas->jam_selesai ?? '-' }} WIB</td>
            </tr>
            <tr>
                ...
                <td style="border:none;">:
                    @foreach($kelas->narasumbers as $n)
                        {{ $n->nama }}@if(!$loop->last), @endif
                    @endforeach
                </td>
            </tr>
        </table>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @foreach($user->nomorInduks as $ni)
                            {{ $ni->nomor_induk }}@if(!$loop->last), @endif
                        @endforeach
                    </td>
                    <td>{{ $user->name }}</td>
                    <td class="text-center">
                        {{ $p?->waktu_presensi_awal ? \Carbon\Carbon::parse($p->waktu_presensi_awal)->format('H:i') : '-' }}
                    </td>
                    <td class="text-center">
                        {{ $p?->waktu_presensi_akhir ? \Carbon\Carbon::parse($p->waktu_presensi_akhir)->format('H:i') : '-' }}
                    </td>
                    <td class="text-center">
                        @if(!$p)
                            Alpha
                        @elseif($p->status_kehadiran == 'hadir_full')
                            Hadir
                        @else
                            Parsial
                        @endif
